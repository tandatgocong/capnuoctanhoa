SELECT DANHBO,HOTEN,SONHA,TENDUONG,MADMA ,KY, NAM FROM TB_DULIEUKHACHHANG WHERE MADMA='TH-04-05'
...
-------------------
UPDATE TB_DULIEUKHACHHANG SET MADMA= NULL  WHERE MADMA='TH-09-06' 
UPDATE TB_DULIEUKHACHHANG SET MADMA='TH-04-01' WHERE DANHBO IN (SELECT DANHBO FROM DMA_TEST) AND MADMA IS NULL
...
SELECT DANHBO,HOTEN, SONHA + ' ' + TENDUONG,MADMA, KY_,NAM
FROM TB_DULIEUKHACHHANG
WHERE MADMA <> 'TH-08-04' and   DANHBO IN (SELECT DANHBO FROM DMA_TEST )
...
SELECT DANHBO,HOTEN, SONHA + ' ' + TENDUONG,MADMA, CONVERT(VARCHAR(10),KY)+'/'+CONVERT(VARCHAR(10),NAM)
FROM TB_DULIEUKHACHHANG 
WHERE    DANHBO  IN (SELECT DANHBO FROM DMA_TEST ) AND MADMA <> 'TH-08-02'
ORDER BY MADMA ASC
...
SELECT * FROM DMA_TEST WHERE MADMA='TH-03-14' AND DANHBO NOT IN (SELECT DANHBO FROM TB_DULIEUKHACHHANG WHERE MADMA='TH-03-14')

SELECT DANHBO FROM DMA_TEST WHERE DANHBO NOT IN ( SELECT DANHBO FROM TB_DULIEUKHACHHANG WHERE  MADMA='TH-08-02')

SELECT DANHBO FROM DMA_TEST WHERE REPLACE(DANHBO,' ','')='13152188483'

UPDATE TB_DULIEUKHACHHANG SET MADMA='TH-07-06' WHERE   DANHBO IN (SELECT DANHBO FROM DMA_TEST ) AND MADMA IS NULL
