...
namespace CAPNUOCTANHOA.DAL.QLDHN
{
     public static class C_BaoThay
    {
        ...
        public static DataTable getBangKeBaoThay(int sobangke) {
            string sql = "SELECT ID_BAOTHAY,DHN_LOAIBANGKE,DHN_SOBANGKE, DHN_DANHBO,HOTEN, SONHA + ' ' +TENDUONG AS 'DIACHI',DHN_NGAYBAOTHAY,DHN_NGAYGAN,DHN_CHITHAN,DHN_CHIGOC,DHN_HIEUDHN,DHN_CODH,DHN_SOTHAN,DHN_CHISO,DHN_LYDOTHAY,DHN_GHICHU,DHN_NGAYCHUYEN ";
        ...
        public static DataTable getBangKeBaoThay_ON(int sobangke)
        {
            ...
            sql += " FROM TB_THAYDHN thay,TB_DULIEUKHACHHANG kh WHERE kh.DANHBO=thay.DHN_DANHBO AND DHN_SOBANGKE='" + sobangke + "'  AND DHN_TODS='" + DAL.SYS.C_USERS._toDocSo + "'  ORDER BY RIGHT(ID_BAOTHAY,3) ";
            return LinQConnection.getDataTable(sql);
        ...
        public static int getMaxBangKe_DC()
        {
        ...
        public static bool deleteBAOTHAY(TB_THAYDHN THAY)
        {
            try
            {
                ...
                db.SubmitChanges();
                LinQConnection.ExecuteCommand("UPDATE TB_DULIEUKHACHHANG SET  HIEUDH='"+THAY.DHN_HIEUDHN+"',SOTHANDH='"+THAY.DHN_SOTHAN+"',  BAOTHAY='False' WHERE DANHBO='" + THAY.DHN_DANHBO + "'");
                return true;
            }
            catch (Exception ex)
            {
