...
namespace CAPNUOCTANHOA.DAL.QLDHN
{
    class C_PhieuKiemTra
    {
        ...
        public static DataTable getListByCode( int tods, int dot, int ky, int nam, string code)
        {
            ...
            sql += "  WHERE  ds.DANHBA=kh.DANHBO AND ds.TODS='"+tods+"' AND ds.NAM="+nam+" AND ds.KY='" + ky + "' AND ds.CODE IN (" + code + ") ";
            if (dot != 0) {
                sql += " AND ds.DOT='"+dot+"'"; 
            }
        ...
        public static DataSet getListHoaDonReport(string danhba, int nam, int ky, int dot)
        {
            ...
            DataSet ds = new DataSet();
            string query2 = "SELECT  kh.*, ds.DOT as 'DOTDS',ds.TODS,ds.MAY,nv.Username  ";
            query2 += " FROM DocSoTH.dbo.DocSo AS ds, CAPNUOCTANHOA.dbo.TB_DULIEUKHACHHANG as kh,DocSoTH.dbo.NHANVIEN nv ";
            query2 += "WHERE nv.MAY=ds.MAY AND ds.DANHBA=kh.DANHBO AND ds.NAM=" + nam + " AND ds.KY=" + ky + " AND ds.DOT=" + dot + " AND ds.DANHBA='" + danhba + "' ";
