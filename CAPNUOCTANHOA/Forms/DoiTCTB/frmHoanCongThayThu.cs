...
{
    public partial class frmHoanCongThayThu : UserControl
    {
        ...
        public void LoadData()
        {
            try
            {
                dataBangKe.DataSource = DAL.DoiTCTB.C_HoanCongThay.getBangKeBaoThayTHU(this.txtSoBangKe.Text.ToUpper());
        ...
        public void Add()
        {
            ...
            thaydh.DHN_HIEUDHN = this.txtHieuDH.Text.ToUpper();
            // thaydh.DHN_LANTHAY = histotyThay.Rows.Count + 1;
            thaydh.DHN_LOAIBANGKE = "TH";
            thaydh.DHN_LYDOTHAY = "THAY THỬ";
            thaydh.DHN_NGAYBAOTHAY = DateTime.Now.Date;
            thaydh.DHN_NGAYGAN = txtNgayGan.Value;
            //  thaydh.DHN_TODS = int.Parse(this.txtSoBangKe.Text.Trim());
            thaydh.DHN_SOTHAN = this.txtSoThan.Text.ToUpper();
            thaydh.DHN_STT = dataBangKe.Rows.Count + 1;
            thaydh.DHN_GHICHU = "THAY THỬ";
            thaydh.DHN_TODS = this.txtSoBangKe.Text.Trim();
        ...
        void LoadThongTinDB()
        {
            ...
            if (sodanhbo.Length == 11)
            {
                ...
                if (khachhang != null)
                {
                    ...
                    txtHieuDH.Text = hieudh != null ? hieudh.TENDONGHO : khachhang.HIEUDH;
        ...
        private void cbKetQua_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbKetQua.SelectedIndex == 1)
            {
                dateHoanLai.Visible = true;
                groupPanel1.Visible = false;
                btKqThay.Visible = true;
            }
            if (cbKetQua.SelectedIndex == 0)
            {
                dateHoanLai.Visible = false;
                btKqThay.Visible = false;
                groupPanel1.Visible = true;
            }
        }

        private void btKqThay_Click(object sender, EventArgs e)
        {
            try
            {
                string ID_BAOTHAY = lbResult.Text.Replace("ID:", "");
                int idbt = int.Parse(ID_BAOTHAY);
              
                TB_THAYDHN thaydh = DAL.QLDHN.C_BaoThay.finByID_BAOTHAY(idbt);
                thaydh.HCT_NGAYGAN = dateHoanLai.Value.Date;
                DAL.QLDHN.C_BaoThay.Update();
                
                TB_VATUTHAY_DHN vtthay = new TB_VATUTHAY_DHN();
                vtthay.ID_BAOTHAY = idbt;
                vtthay.DOTTHAY = txtSoBangKe.Text;
                vtthay.STT = 1;
                vtthay.MAVT = "NHANCONG";
                vtthay.TENVT = "NHÂN CÔNG";
                vtthay.DVT = "Lần";
                vtthay.SOLUONG = 2;
                vtthay.GHICHU = "Thay thử";
                vtthay.CREATEBY = DAL.SYS.C_USERS._userName;
                vtthay.CREATEDATE = DateTime.Now;
                DAL.DoiTCTB.C_HoanCongThay.InsertVatTuThay(vtthay);
                MessageBox.Show(this, "HOÀN CÔNG THÀNH CÔNG .", "..: Thông Báo :..", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception)
            {
                MessageBox.Show(this, "HOÀN CÔNG KHÔNG THÀNH CÔNG .", "..: Thông Báo :..", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
