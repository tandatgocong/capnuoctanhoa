    ...
    public partial class tab_TieuThuThap_dot : UserControl
    {
        ...
        private void btThem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // ky hien tai
                DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KYNAY_DOT(nam.ToString(), ky,dot);
                // ky truoc
                if (ky == 1)
                {
                    DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KYTRUOC_DOT((nam - 1) + "",12,dot);
                }
                else
                {
                    DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KYTRUOC_DOT(nam.ToString(), ky - 1,dot);
                }

                // nam truoc
                DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KY_NAMTRUOC_DOT((nam - 1) + "", ky,dot);
        ...
        private void sanluongToDS_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (sanluongToDS.CurrentCell.OwningColumn.Name == "TENTO")
            {
                ...
                try
                {
                    ...
                    // ky hien tai
                    DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KYNAY_MAY_DOT(nam.ToString(), ky, dot);
                    // ky truoc
                    if (ky == 1)
                    {
                        DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KYTRUOC_MAY_DOT((nam - 1) + "", 12, dot);
                    }
                    else
                    {
                        DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KYTRUOC_MAY_DOT(nam.ToString(), ky - 1, dot);
                    }

                    // nam truoc
                    DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_KY_NAMTRUOC_MAY_DOT((nam - 1) + "", ky, dot);

                    // CAP NHAT SO LIEU 
                    DAL.QLDHN.C_BaoCaoTieuThuThap.CAPNHATSOLIEU_BAOCAO_SANLUONG_TANGGIAM_MAY_DOT();
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            rp.SetParameterValue("tenbk", "BÁO CÁO TIÊU THỤ THẤP KỲ  " + cbKyDS.Items[cbKyDS.SelectedIndex].ToString() + "/" + txtNam.Text.Trim());
