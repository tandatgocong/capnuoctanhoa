...
-- THU TIEN
CREATE PROC UPDATEHIEUDHN
AS
	UPDATE HOADON SET Quan=DLKH_HUY.QUAN  ,Phuong=DLKH_HUY.PHUONG ,CoDH=DLKH_HUY.CODH ,MaDMA=DLKH_HUY.MADMA,HIEULUC=DLKH_HUY.HIEULUCHUY, NAMLD=YEAR(DLKH_HUY.NGAYTHAY),HIEUDH=DLKH_HUY.HIEUDH  
	FROM DLKH_HUY WHERE   HOADON.DANHBA= DLKH_HUY.DANHBO AND HOADON.HIEUDH IS NULL
	  
	UPDATE HOADON SET Quan=DLKH.QUAN ,Phuong=DLKH.PHUONG ,CoDH=DLKH.CODH  ,MaDMA=DLKH.MADMA,HIEULUC=(CONVERT(VARCHAR,DLKH.KY)+'/'+CONVERT(VARCHAR,DLKH.NAM) ), NAMLD=YEAR(DLKH.NGAYTHAY),HIEUDH=DLKH.HIEUDH   
	FROM DLKH WHERE    HOADON.DANHBA= DLKH.DANHBO  AND HOADON.HIEUDH IS NULL

	
CREATE PROCEDURE THONGKEDHN_HD
	@KY INT,
	@NAM INT
AS
	UPDATE TB_THONGKEDHN SET 
	...
	FROM (
			SELECT LEFT(replace(kh.HIEUDH,' ',''),3) AS HIEUDH,
				...
				COUNT(CASE WHEN CODH=150 THEN 1 ELSE NULL END) AS CO150,
				COUNT(CASE WHEN CODH=200 THEN 1 ELSE NULL END) AS CO200,
				COUNT(CASE WHEN CODH=15 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO15,
				COUNT(CASE WHEN CODH=20 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO20,
				COUNT(CASE WHEN CODH=25 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO25,
				COUNT(CASE WHEN CODH=40 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO40,
				COUNT(CASE WHEN CODH=50 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO50,
				COUNT(CASE WHEN (CODH=75 OR  CODH= 80) AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4)  THEN 1 ELSE NULL END) AS NHOCO80,
				COUNT(CASE WHEN CODH=100 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO100,
				COUNT(CASE WHEN CODH=150 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO150,
				COUNT(CASE WHEN CODH=200 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) < 4) THEN 1 ELSE NULL END) AS NHOCO200,
				COUNT(CASE WHEN CODH=15 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4) THEN 1 ELSE NULL END) AS LONCO15,
				COUNT(CASE WHEN CODH=20 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4) THEN 1 ELSE NULL END) AS LONCO20,
				COUNT(CASE WHEN CODH=25 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4) THEN 1 ELSE NULL END) AS LONCO25,
				COUNT(CASE WHEN CODH=40 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4) THEN 1 ELSE NULL END) AS LONCO40,
				COUNT(CASE WHEN CODH=50 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4) THEN 1 ELSE NULL END) AS LONCO50,
				COUNT(CASE WHEN (CODH=75 OR  CODH= 80) AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4)  THEN 1 ELSE NULL END) AS LONCO80,
				COUNT(CASE WHEN CODH=100 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4) THEN 1 ELSE NULL END) AS LONCO100,
				COUNT(CASE WHEN CODH=150 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD) >=4) THEN 1 ELSE NULL END) AS LONCO150,
				COUNT(CASE WHEN CODH=200 AND ((YEAR(CURRENT_TIMESTAMP) - NAMLD ) >=4) THEN 1 ELSE NULL END) AS LONCO200
			FROM dbo.HOADON_TH kh
			WHERE kh.NAM=2017 AND kh.KY=4
			GROUP BY  LEFT(replace(kh.HIEUDH,' ',''),3)
	) as t2
	WHERE TB_THONGKEDHN.MADHN=t2.HIEUDH 	
GO
...
CREATE VIEW TONGDHN_DMA
as
SELECT MADMA, COUNT(*) AS TONG,
COUNT(CASE WHEN CODH=15 THEN 1 ELSE NULL END) AS CO15,
	COUNT(CASE WHEN CODH=20 THEN 1 ELSE NULL END) AS CO20,
	COUNT(CASE WHEN CODH=25 THEN 1 ELSE NULL END) AS CO25,
	COUNT(CASE WHEN CODH=40 THEN 1 ELSE NULL END) AS CO40,
...
SELECT  * FROM TONGDHN_DMA WHERE MADMA='TH-01-01'  
...
select  LEFT(replace(HIEUDH,' ',''),3) ,COUNT(*) from TB_DULIEUKHACHHANG 
where YEAR(NGAYKIEMDINH)=2014  group by LEFT(replace(HIEUDH,' ',''),3) 

select CASE WHEN LEFT(replace(HIEUDH,' ',''),3)='KEN' THEN 'C' ELSE 'B' END AS 'CAP' ,COUNT(*) 
from TB_DULIEUKHACHHANG where YEAR(NGAYKIEMDINH)=2014 and CODH=15
GROUP BY CASE WHEN LEFT(replace(HIEUDH,' ',''),3)='KEN' THEN 'C' ELSE 'B' END
