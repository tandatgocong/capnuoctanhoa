...
namespace CAPNUOCTANHOA.DAL.GNKDT
{
    class C_GNKDT
    {
        ...
        public static DataTable getThongTinDMAByHoaDon(string madma, string ky, string nam)
        {
            ...
            query += "       FROM (SELECT LOTRINH,DANHBO,HOPDONG,HOTEN,SONHA,TENDUONG,CODH,GIABIEU,DINHMUC, (CONVERT(VARCHAR,KY)+'/'+CONVERT(VARCHAR,NAM) ) as 'HIEULUC',YEAR(NGAYTHAY) AS 'NAMLD' ";
            query += "              FROM TB_DULIEUKHACHHANG WHERE NAM<=" + nam + " AND KY_<=" + ky + " AND MADMA='" + madma + "'";
            query += "              UNION ";
            query += "              SELECT LOTRINH,DANHBO,HOPDONG,HOTEN,SONHA,TENDUONG,CODH,GIABIEU,DINHMUC ,( N'Hủy ' + HIEULUCHUY) as 'HIEULUC',YEAR(NGAYTHAY) AS 'NAMLD' ";
        ...
        public static DataTable getThongTinDMAByHandheld(string madma, string ky, string nam)
        {
            ...
            string query = " SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY LOTRINH  DESC) [STT], LOTRINH,DANHBO,HOPDONG,HOTEN,SONHA,TENDUONG,CODH,GIABIEU,DINHMUC, HIEULUC ,CHUKYDS as 'CHUKY',NAMLD, tb2.CODE,tb2.TIEUTHU as 'LNCC',(convert(float,tb2.TIEUTHU)/tb1.CHUKYDS) as 'TBTT'    ";
            query += "      FROM ( SELECT LOTRINH,DANHBO,HOPDONG,HOTEN,SONHA,TENDUONG,CODH,GIABIEU,DINHMUC, (CONVERT(VARCHAR,KY)+'/'+CONVERT(VARCHAR,NAM) ) as 'HIEULUC',CHUKYDS,YEAR(NGAYTHAY) AS 'NAMLD'            ";
            query += "  		FROM TB_DULIEUKHACHHANG WHERE  NAM<="+nam+" AND KY_<="+ky+" AND MADMA='" + madma + "'";
            query += "  		 UNION      ";
            query += "  		SELECT LOTRINH,DANHBO,HOPDONG,HOTEN,SONHA,TENDUONG,CODH,GIABIEU,DINHMUC ,( N'Hủy ' + HIEULUCHUY) as 'HIEULUC',CHUKYDS,YEAR(NGAYTHAY) AS 'NAMLD'           ";
        ...
        public static DataTable getThongTinDMAByHandheld_Thay(string madma, string ky, string nam)
        {
            ...
            string query = " SELECT DISTINCT  ROW_NUMBER() OVER (ORDER BY LOTRINH  DESC) [STT], LOTRINH,DANHBO,HOPDONG,HOTEN,SONHA,TENDUONG,CODH,GIABIEU,DINHMUC, HIEULUC ,CHUKYDS as 'CHUKY', tb2.CODE,tb2.TIEUTHU as 'LNCC',(convert(float,tb2.TIEUTHU)/tb1.CHUKYDS) as 'TBTT'    ";
