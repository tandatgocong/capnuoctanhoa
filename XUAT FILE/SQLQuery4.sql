...
SELECT SUBSTRING(LOTRINH,1,2) as 'DOT',kh.LOTRINH, kh.DANHBO, kh.HOPDONG, kh.HOTEN, kh.SONHA, kh.TENDUONG,P.TENPHUONG, q.TENQUAN,kh.HIEUDH, kh.CODH,SOTHANDH , VITRIDHN, convert(varchar(20),kh.NGAYTHAY,103) AS NAM, convert(varchar(20),kh.NGAYKIEMDINH,103) AS NGAYKIEMDINH
FROM  TB_DULIEUKHACHHANG kh,  PHUONG p, QUAN q
WHERE kh.QUAN = q.MAQUAN AND q.MAQUAN=p.MAQUAN AND kh.PHUONG=p.MAPHUONG
AND YEAR(NGAYTHAY)<=2010 AND (BAOTHAY IS NULL OR BAOTHAY=0)
...
FROM  TB_DULIEUKHACHHANG kh,  PHUONG p, QUAN q
WHERE kh.QUAN = q.MAQUAN AND q.MAQUAN=p.MAQUAN AND kh.PHUONG=p.MAPHUONG 
AND CODH >=25 and NGAYKIEMDINH is not null
...
WHERE  NGAYKIEMDINH is not null
order by kh.LOTRINH asc
...
SELECT SUBSTRING(LOTRINH,1,2) as 'DOT',kh.LOTRINH, kh.DANHBO, kh.HOPDONG, kh.HOTEN, kh.SONHA, kh.TENDUONG,P.TENPHUONG,kh.HIEUDH, kh.CODH,SOTHANDH, YEAR(kh.NGAYTHAY) AS 'NAM' , VITRIDHN,hd.GIABIEU,hd.DINHMUC,hd.CSCU, hd.cODE,hd.LNCC,hd.NHANVIEN
FROM  TB_DULIEUKHACHHANG kh,HOADONTH07_2014 hd,  PHUONG p, QUAN q
WHERE kh.DANHBO=hd.DANHBO and kh.QUAN = q.MAQUAN AND q.MAQUAN=p.MAQUAN AND kh.PHUONG=p.MAPHUONG
AND p.MAPHUONG = '15' AND p.MAQUAN='23'
ORDER BY  LOTRINH ASC
...
FROM  TB_DULIEUKHACHHANG kh,HOADONTH09_2014 hd,  PHUONG p, QUAN q
WHERE kh.DANHBO=hd.DANHBO and kh.QUAN = q.MAQUAN AND q.MAQUAN=p.MAQUAN AND kh.PHUONG=p.MAPHUONG
AND p.MAPHUONG = '01' AND p.MAQUAN='31'   AND LNCC=0
ORDER BY  LOTRINH ASC
...
FROM  TB_DULIEUKHACHHANG kh,HOADONTH10_2014 hd,  PHUONG p, QUAN q
WHERE kh.DANHBO=hd.DANHBO and kh.QUAN = q.MAQUAN AND q.MAQUAN=p.MAQUAN AND kh.PHUONG=p.MAPHUONG
AND p.MAPHUONG = '15' AND p.MAQUAN='23'   AND LNCC=0
ORDER BY  LOTRINH ASC
