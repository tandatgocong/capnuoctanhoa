UPDATE HOADONTH08 SET  SONHA= t2.SONHA, tenduong=t2.TENDUONG
FROM TB_DULIEUKHACHHANG t2
WHERE HOADONTH08.DANHBO = t2.DANHBO

UPDATE HOADONTH09 SET  PHUONG= t2.PHUONG, QUAN=t2.QUAN, Khu= t2.LOTRINH
FROM TB_DULIEUKHACHHANG t2
WHERE HOADONTH09.DANHBO = t2.DANHBO
...
UPDATE HOADONTH07 SET  NHANVIEN = t2.NAME
FROM TB_NHANVIENDOCSO t2
WHERE CONVERT(int,SUBSTRING(Khu,3,2))= t2.MAYDS AND HOADONTH07.Code='K'
...
SELECT p.MAQUAN,p.MAPHUONG, p.TENPHUONG, q.TENQUAN, COUNT(*), SUM(CONVERT(FLOAT,Field29))
FROM HOADONTH08 kh, PHUONG p, QUAN q
WHERE  kh.QUAN = q.MAQUAN AND q.MAQUAN=p.MAQUAN AND kh.PHUONG=p.MAPHUONG AND  Field29=0
GROUP BY p.MAQUAN,p.MAPHUONG, p.TENPHUONG, q.TENQUAN
ORDER BY  p.MAQUAN,p.MAPHUONG ASC
...
SELECT p.MAQUAN,p.MAPHUONG, p.TENPHUONG, q.TENQUAN, COUNT(*)
FROM HOADONTH08 kh, PHUONG p, QUAN q
WHERE kh.DINHMUC=0 AND kh.QUAN = q.MAQUAN AND q.MAQUAN=p.MAQUAN AND kh.PHUONG=p.MAPHUONG 
GROUP BY p.MAQUAN,p.MAPHUONG, p.TENPHUONG, q.TENQUAN
ORDER BY  p.MAQUAN,p.MAPHUONG ASC
