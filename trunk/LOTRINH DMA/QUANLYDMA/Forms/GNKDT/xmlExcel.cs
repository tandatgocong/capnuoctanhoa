...
namespace CAPNUOCTANHOA.Forms.GNKDT
{
    class xmlExcel
    {
        public void BuildWorkbook(string fileName, DataTable ds, string nam, string ky, string madma)
        {
            try
            {
                using (SpreadsheetDocument s = SpreadsheetDocument.Create(fileName, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    //int[] width_column = { 5, 13, 6, 13, 30, 17, 20, 8, 10, 6, 7, 9, 20 };o 
                    // tạo width cho từng cột , màng width_column theo thứ tự của cột header
                    int[] width_column = { 5, 13, 13, 30, 17, 20, 6, 8, 10, 7, 6, 9, 20 };
        ...
        private Row CreateHeaderRow(UInt32 index, DataTable dt)
        {
            ...
            {
                namecolumn = col.ColumnName;
                if (col.ColumnName == "LNCC")
                {
                    namecolumn = "TIEUTHU";
                }
                if (col.ColumnName == "TBTT")
                {
                    namecolumn = "BINHQUAN";
                }

                Cell cell = CreateTextCell(namecolumn);
                cell.StyleIndex = 10;
                r.Append(cell);
            }
            return r;
